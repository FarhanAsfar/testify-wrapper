...
type Config struct {
	...
	// If MaxProcs > 0, that value is used as the floor passed to maxprocs.Min(). The actual GOMAXPROCS will be at least MaxProcs but automaxprocs may set it higher if the CPU quota allows
	MaxProcs int
}
...
func Configure(cfg Config) {
	...
	if err != nil {
		// Fall back to NumCPU but log a warning
		fmt.Fprintf(os.Stderr, "testify-wrapper: automaxprocs failed (%v), using runtime.NumCPU()\n", err)
